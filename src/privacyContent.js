  ...
  pt: {
    ...
    sections: [
      ...
      },
      {
        ...
        body: 'O Jesus\' Corner tem um plano de assinatura premium opcional — a maior parte do app é grátis pra sempre; veja o site pra detalhes de tudo que cada plano libera. Se você assinar, os dados de pagamento (número do cartão, validade, etc.) são inseridos e processados diretamente pelo Stripe, nosso provedor de pagamentos — nunca vemos nem armazenamos esses dados em nossos próprios sistemas. Compartilhamos com o Stripe apenas o necessário pra processar a cobrança, como seu e-mail e um identificador da sua assinatura. Em nosso próprio banco de dados guardamos só o status da sua assinatura (ativa, cancelada, etc.), o plano escolhido (mensal ou anual) e até quando ela vale — nunca os dados do seu cartão. O preço exibido (em reais ou em dólares) é determinado automaticamente pelo país detectado a partir do seu endereço IP no momento da assinatura. Você pode cancelar quando quiser direto pelo app (Perfil → Gerenciar assinatura), o que te leva a um painel hospedado pelo próprio Stripe, onde também dá pra trocar o cartão cadastrado e baixar recibos.',
      },
      {
  ...
  en: {
    ...
    sections: [
      ...
      },
      {
        ...
        body: 'Like any website, when you access Jesus\' Corner some technical data passes through our hosting infrastructure (Vercel Inc.) — such as IP address, browser type, and access time — used to deliver content, detect your billing currency (Brazilian reais or US dollars, based on your detected country), and for security. We also load fonts through Google Fonts, which sends a request to Google\'s servers, use Supabase Inc. (see section 4) for authentication and data storage, and, if you subscribe to the premium plan, use Stripe Inc. to process payment (see section 9). None of this technical data is combined with your account content for advertising purposes.',
      },
      {
      ...
      },
      {
        ...
        body: 'Jesus\' Corner has an optional premium subscription plan — most of the app is free forever; see the site for details on everything each plan unlocks. If you subscribe, payment data (card number, expiration date, etc.) is entered and processed directly by Stripe, our payment provider — we never see or store that data on our own systems. We share with Stripe only what\'s needed to process billing, such as your email and a subscription identifier. In our own database, we store only your subscription status (active, canceled, etc.), the plan you chose (monthly or annual), and its current expiration — never your card details. The price shown (in Brazilian reais or US dollars) is determined automatically from the country detected via your IP address at the time you subscribe. You can cancel anytime from within the app (Profile → Manage subscription), which takes you to a portal hosted directly by Stripe, where you can also update your card on file and download receipts.',
      },
      {
